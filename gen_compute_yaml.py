#!/usr/bin/env python3
"""
GB300 Compute YAML Generator
Generates compute_bmc.yaml, compute_hmc.yaml, and compute_mcu.yaml files
...
def display_intro():
    ...
    print("=" * 70)
    print("GB300 Compute YAML Generator")
...
def parse_system_name_range(name_input: str) -> List[str]:
    ...
    Supports formats:
    - Single name: CP-01
    - Comma-separated list: CP-01,CP-02,CP-03
    - Range format 1: CP-[01-18]
    - Range format 2: CP-01..CP-18
    """
    ...
    # Check for bracket range format (e.g., CP-[01-18])
    ...
    # Check for double-dot range format (e.g., CP-01..CP-18)
    if '..' in name_input:
...
def find_firmware_files(firmware_dir: str) -> Tuple[str, str, str]:
    ...
    if not hmc_file:
        ...
        if remaining_files:
            ...
            while True:
                try:
                    choice = int(input("Select HMC firmware file number: ")) - 1
                    if 0 <= choice < len(remaining_files):
                        hmc_file = remaining_files[choice]
                        break
                    else:
                        print("Invalid selection. Please try again.")
                except ValueError:
    ...
    if not mcu_file:
        ...
        if remaining_files:
            ...
            while True:
                try:
                    choice = int(input("Select MCU firmware file number: ")) - 1
                    if 0 <= choice < len(remaining_files):
                        mcu_file = remaining_files[choice]
                        break
                    else:
                        print("Invalid selection. Please try again.")
                except ValueError:
    ...
    return bmc_file, hmc_file, mcu_file
...
def check_ip_conflicts(new_ips: List[str], existing_yaml_files: List[str], 
    ...
    if conflicts:
        ...
        print("Please use different IP addresses for your compute configuration.")
        return True
...
def run_reachability_test():
    ...
    try:
        ...
        if not os.path.exists('test_compute_reachability.py'):
            print("Error: test_compute_reachability.py not found in current directory.")
            print("Please ensure the reachability test script is in the same directory.")
            return
        
        # Run the reachability test script
        print("\nLaunching reachability test...")
        result = subprocess.run([sys.executable, 'test_compute_reachability.py'], 
                              check=False)
...
def main():
    ...
    try:
        ...
        bmc_file, hmc_file, mcu_file = find_firmware_files(firmware_dir)
        
        # Validate all files were found/selected
        if not all([bmc_file, hmc_file, mcu_file]):
            print("Error: Could not find all required firmware files.")
            return
        
        # Create full paths
        bmc_path = os.path.join(firmware_dir, bmc_file).replace('\\', '/')
        hmc_path = os.path.join(firmware_dir, hmc_file).replace('\\', '/')
        mcu_path = os.path.join(firmware_dir, mcu_file).replace('\\', '/')
        
        print(f"\nSelected firmware files:")
        print(f"BMC: {bmc_file}")
        print(f"HMC: {hmc_file}")
        print(f"MCU: {mcu_file}")
        
        # Check for IP conflicts with switch YAML files
        switch_yaml_files = ['switch_bmc.yaml', 'switch_bios.yaml', 'switch_cpld.yaml']
        if check_ip_conflicts(ip_list, switch_yaml_files, "switch"):
        ...
        bmc_content = generate_yaml_content(ip_list, name_list, username, password, 
                                          bmc_path, {})
        with open('compute_bmc.yaml', 'w') as f:
            f.write(bmc_content)
        print("Generated: compute_bmc.yaml")
        
        # HMC YAML (with HGX_Chassis_0 target)
        hmc_content = generate_yaml_content(ip_list, name_list, username, password, 
                                          hmc_path, {"Targets": ["/redfish/v1/Chassis/HGX_Chassis_0"]})
        with open('compute_hmc.yaml', 'w') as f:
            f.write(hmc_content)
        print("Generated: compute_hmc.yaml")
        
        # MCU YAML (with HGX_Chassis_0 target)
        mcu_content = generate_yaml_content(ip_list, name_list, username, password, 
                                          mcu_path, {"Targets": ["/redfish/v1/Chassis/HGX_Chassis_0"]})
        with open('compute_mcu.yaml', 'w') as f:
            f.write(mcu_content)
        print("Generated: compute_mcu.yaml")
