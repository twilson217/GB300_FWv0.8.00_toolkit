#!/usr/bin/env python3
"""
GB300 Switch BMC Reset Script
Executes BMC resets via Redfish API for all IP addresses in switch_*.yaml files.
...
def validate_switch_yaml_files() -> Tuple[List[Dict], Set[str]]:
    ...
    if not switch_files:
        raise BMCResetError("No switch_*.yaml files found in current directory or subdirectories")
...
def validate_no_compute_conflicts(switch_ips: Set[str]) -> None:
    ...
    if conflicts:
        raise BMCResetError(f"IP address conflicts detected between switch and compute YAML files: {sorted(conflicts)}")
...
def main():
    """Main program flow."""
    print("GB300 Switch BMC Reset Tool")
    print("=" * 40)
