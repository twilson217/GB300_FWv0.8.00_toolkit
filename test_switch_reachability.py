#!/usr/bin/env python3
"""
GB300 Switch Reachability Test
Tests ping and TCP port 443 connectivity to IP addresses from generated switch YAML files.
"""
...
def load_yaml_files() -> List[str]:
    """Load IP addresses from all switch YAML files."""
    yaml_files = ['switch_bmc.yaml', 'switch_bios.yaml', 'switch_cpld.yaml']
    ip_addresses = set()
    ...
    if not ip_addresses:
        ...
        print("Make sure the switch_*.yaml files exist and contain valid data.")
        sys.exit(1)
...
def main():
    """Main program flow."""
    print("GB300 Switch Reachability Test")
    print("=" * 40)
