#!/usr/bin/env python3
"""
GB300 Switch Power Cycle Script
Executes power off followed by power cycle via Redfish API for all IP addresses in switch_*.yaml files.
...
def validate_switch_yaml_files() -> Tuple[List[Dict], Set[str]]:
    """
    Validate all switch YAML files and ensure IP address consistency.
    Returns tuple of (all_targets, all_ips).
    """
    print("=" * 60)
    print("VALIDATING SWITCH YAML FILES")
    print("=" * 60)
    
    # Find switch YAML files
    switch_files = find_yaml_files('switch_')
    if not switch_files:
        raise PowerCycleError("No switch_*.yaml files found in current directory or subdirectories")
    ...
    # Validate IP consistency across all switch files
    print(f"\nValidating IP address consistency across switch files...")
    reference_file = switch_files[0]
    ...
    print(f"✓ All switch YAML files have consistent IP addresses")
    print(f"✓ Total unique IP addresses: {len(reference_ips)}")
    
    return all_targets, reference_ips


def validate_no_compute_conflicts(switch_ips: Set[str]) -> None:
    """Validate that switch IPs don't conflict with compute YAML files."""
    print(f"\nValidating no conflicts with compute YAML files...")
    
    # Find compute YAML files
    compute_files = find_yaml_files('compute_')
    if not compute_files:
        print("  No compute YAML files found - skipping conflict check")
        return
    
    print(f"  Found {len(compute_files)} compute YAML files")
    
    # Extract all compute IPs
    compute_ips = set()
    for file_path in compute_files:
        try:
            ...
            targets = extract_targets_from_yaml(yaml_data, file_path)
            file_ips = {target['BMC_IP'] for target in targets}
            compute_ips.update(file_ips)
        except Exception as e:
            print(f"  Warning: Could not process {file_path}: {e}")
            continue
    
    # Check for conflicts
    conflicts = switch_ips & compute_ips
    if conflicts:
        raise PowerCycleError(f"IP address conflicts detected between switch and compute YAML files: {sorted(conflicts)}")
    
    print(f"✓ No IP conflicts between switch ({len(switch_ips)}) and compute ({len(compute_ips)}) files")
...
def get_user_confirmation(targets: List[Dict], switch_ips: Set[str]) -> bool:
...
def main():
    """Main program flow."""
    print("GB300 Switch Power Cycle Tool")
    print("=" * 40)
    
    try:
        # Validate YAML files and extract data
        all_targets, switch_ips = validate_switch_yaml_files()
        
        # Validate no conflicts with compute files
        validate_no_compute_conflicts(switch_ips)
        ...
        if not get_user_confirmation(all_targets, switch_ips):
